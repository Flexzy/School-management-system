<?php 
	...
	$get_sc_terms = $db->query("SELECT * FROM academic_year WHERE active=1 LIMIT 1");
	if($get_sc_terms->num_rows) {
		$row = $get_sc_terms->fetch_assoc();
		$sc_session = escape($row['session_year']);
		$sc_term = escape($row['term']);
	}
